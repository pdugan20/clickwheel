...
- Default to exploration for interface iterations, prototypes, and small changes. Make focused edits directly; do not require a formal spec, separate plan, worktree, or TDD.
...
- Ask before syncing or ejecting a device, deleting playlists, rewriting media metadata, publishing a release, or making any other live mutation. A user request naming the exact mutation and target counts as approval for that operation.
...
12. **MCP destructive tools advertise risk but do not enforce authorization** — mutation tools (`delete_playlist`, `sync_playlist_to_ipod`, the `add_*`/`remove_*` family, etc.) carry the `destructiveHint=true` annotation (the `DESTRUCTIVE` preset in `_runtime.py`). Keep that annotation on every mutating tool, but treat it as client-facing metadata rather than a server-side security boundary. The server does **not** call `Context.elicit()` and there is no `confirm` parameter. Before calling a destructive tool, summarize the impact (track count, target, names) and obtain approval unless the user's request already names the exact mutation and target. (`ctx: Context` is used only for `report_progress`.)
