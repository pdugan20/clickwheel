...
- **Round 4 slash-command parsing** — `/clickwheel:build_playlist vibe:"late-night indie folk" target_minutes:45` failed with a Pydantic validation error: Code's slash-command parser whitespace-splits without honoring quotes, so `indie` was assigned to `target_minutes`. Empirically confirmed our schema is correct (multi-word vibe works fine over the MCP wire when the client sends args as proper key/value pairs). Researched: known Claude Code parser bug tracked in closed-as-inactive issues (#14210, #4125, #30733). Documented the natural-language invocation pattern in the prompt's description; not filing upstream per user.
- **Round 6.0 — Desktop dev install path** — In-tree `.venv` under `~/Documents/` failed to launch from Desktop's sandbox: `PermissionError: Operation not permitted: pyvenv.cfg`. macOS 15 applies a `com.apple.provenance` xattr to files in `Documents`, and Desktop's sandbox refuses sibling-file reads on them; rewind works because Node doesn't read sibling files at boot. Fix: install via `pipx install --editable` so the venv lives in `~/.local/pipx/` (outside Documents). README's `pipx` recipe already lands users in the correct location; only dev-install from a clone in Documents is affected.
- **Round 6.2 — Desktop dropped structuredContent** — Tools like `list_albums_by_artist` returned a thin text summary in `content` ("3 albums by Taylor Swift") and the actual records in `structuredContent`. Worked in Code (which surfaces both to the model) but failed in Desktop (which only feeds content blocks to the model). Model literally said "the album list came back as just a count." Fixed `render()` to dual-emit: serialize the structured payload to a second `TextContent` block alongside the summary while keeping `structuredContent`. The MCP spec endorses this for compat. After fix, Desktop successfully chained `list_albums_by_artist` → `list_tracks_by_album` → `create_playlist` end-to-end.
- **Round 6.3 — Desktop destructive gating differs from Code** — `destructiveHint=true` does NOT trigger a per-call native Allow/Deny prompt in Desktop. Desktop's gating model is per-tool first-use approval (one popup the first time `delete_playlist` is invoked in a conversation). Combined with our `instructions` block telling the model to summarize impact in chat before destructive calls, the safety chain is: model self-confirms → user replies "yes" → first-time tool-load approval (if applicable) → tool runs. Weaker than Code's per-call gate, but acceptable. No change.
- **Round 6.4 — Desktop has no UI for MCP prompts from local stdio servers** — `build_playlist` does not appear in Desktop's `/` picker (which surfaces Skills only), Connectors menu (cloud MCP only), or Plugins (greyed out). Local stdio servers configured via `claude_desktop_config.json` only surface their tools (under "Tool access — Tools already loaded"); their prompts are effectively unreachable via Desktop UI. Workaround: natural-language invocation, where the model reads the prompt body and follows the workflow. Same shape as Code's parser bug — both clients break the prompt path differently.
