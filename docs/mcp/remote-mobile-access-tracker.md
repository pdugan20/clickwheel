...
iOS app. The *why* and design rationale live in the brief
([`remote-mobile-access.md`](remote-mobile-access.md)); the owner-side runbook
(commands + dashboard clicks) is in
[`deploy/README.md`](deploy/README.md). This is the *what, in order, and what's
blocking it*.
...
OAuth code on our side. (Not a *self-hosted* Access app; that shows a browser
...
*separate* (server identity in the handshake) and does **not** drive the
...
→ Mac behind Cloudflare Access**, which gates the *entire* hostname — hence the
