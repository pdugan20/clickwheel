...
- **Mutation tools advertise their risk.** They carry the MCP `destructiveHint`
  annotation, which compatible clients can use to request approval. This is
  client behavior, not server-side authorization, so review the target and
  impact before allowing a mutation.
