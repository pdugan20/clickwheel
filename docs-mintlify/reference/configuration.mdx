...
Environment variables override the config file. Most use the setting name in
`UPPER_CASE` (for example, `MUSIC_DIR`). The `library_*` and `transcode_*`
settings add a `CLICKWHEEL_` prefix.
...
| `transcode_dir` | `~/.clickwheel/transcoded` | Output folder for MP3 copies created by [`convert`](/guides/convert-flac). |
| `transcode_bitrate` | `320` | MP3 CBR bitrate in kbps for conversion. |
...
| `ALLOWED_HOSTS` | _(unset)_ | Host header(s) to allow behind a tunnel (e.g. `mcp.example.com`). |
