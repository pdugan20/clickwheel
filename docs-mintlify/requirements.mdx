...
| FLAC | Convert first | Stock firmware can't play it. [`clickwheel convert`](/guides/convert-flac) creates a separate MP3 copy; sync never transcodes automatically. |

<Note>
  `ffmpeg` is required only for FLAC conversion. Install it with
  `brew install ffmpeg` before running `clickwheel convert`.
</Note>
